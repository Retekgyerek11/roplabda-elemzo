...
function ensureSetBucket(stats, setNo){
  if (!setNo) return null;
  if (!stats.bySet[setNo]) stats.bySet[setNo] = {
    attack:{n:0,kill:0,err:0,blk:0}, recv:{n:0,good:0,err:0},
    serve:{n:0,ace:0,err:0}, block:{n:0,kill:0}, rally:{won:0,lost:0},
  };
  return stats.bySet[setNo];
}
...
function computeTeamStats(matchList, teamName){
  ...
  const stats = {
    ...
    bySet: {},        // setNo -> {attack:{n,kill,err,blk}, recv:{n,good,err}, serve:{n,ace,err}, block:{n,kill}, rally:{won,lost}}
  };
  ...
  matchList.forEach(({filename, rawText})=>{
    ...
    // "**Nset" lines CLOSE set N — they carry that set's end time and score, so
    // the actions above the marker are the ones that belong to it. Counting
    // starts at 1 and the marker moves us on to the next set; reading it as an
    // opening marker shifts every action one set late and loses the last set.
    let currentSet = 1;
    let lineup = [];
    ...
    for (const raw of lines){
      ...
      const setMark = line.match(/^\*\*(\d)set/);
      if (setMark){ currentSet = parseInt(setMark[1],10) + 1; continue; }
      ...
      if (skill==='R'){
        ...
        const sbR = ensureSetBucket(stats, currentSet);
        if (sbR){ sbR.recv.n++;
          if (grade==='#'||grade==='+') sbR.recv.good++;
          else if (grade==='=') sbR.recv.err++; }
        lastRecvGrade = grade;
        ...
        bump(o.grades, grade); o.total++;
        const sbA = ensureSetBucket(stats, currentSet);
        if (sbA){ sbA.attack.n++;
          if (grade==='#') sbA.attack.kill++;
          else if (grade==='=') sbA.attack.err++;
          else if (grade==='/') sbA.attack.blk++; }
        ...
        bump(o.grades, grade); o.total++;
        const sbS = ensureSetBucket(stats, currentSet);
        if (sbS){ sbS.serve.n++;
          if (grade==='#') sbS.serve.ace++; else if (grade==='=') sbS.serve.err++; }
        ...
        bump(o.grades, grade); o.total++;
        const sbB = ensureSetBucket(stats, currentSet);
        if (sbB){ sbB.block.n++; if (grade==='#') sbB.block.kill++; }
      }
    }
  });
